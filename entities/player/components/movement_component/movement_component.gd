...
func apply_input_movement(input_dir: Vector2, target_speed: float)-> void:
...
func apply_pure_friction(delta: float, custom_friction: float = -1.0)-> void:
	# 如果没有传入特定的摩擦力，就使用默认的正常摩擦力
	var current_friction: float = friction if custom_friction < 0.0 else custom_friction
